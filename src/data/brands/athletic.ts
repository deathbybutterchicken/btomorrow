export const HEADINGS = {
  line1: "PIONEERING CRAFT",
  line2: "NON-ALCOHOLIC REVOLUTION",
  mainHeading: "ATHLETIC.BREWING",
  imageAlt: "Athletic Brewing Non-Alcoholic Craft Beer Products",
};

export const executives = [
  {
    name: "John Walker",
    title: "Co-Founder and Head Brewer",
    image:
      "https://cdn.brewersassociation.org/wp-content/uploads/2022/08/03104741/John-Walker.jpg",
    experience: [
      "Former Head Brewer at Second Street Brewery",
      "Over a decade of experience in craft brewing",
    ],
    entrepreneurial: [
      "Co-founded Athletic Brewing Company",
      "Developed award-winning non-alcoholic beers",
    ],
    linkedin: "https://www.linkedin.com/in/john-walker-253b26162/",
  },
  // Add more executives if available
];

export const founderData = {
  name: "Bill Shufelt",
  title: "Co-Founder and CEO",
  education:
    "Bachelor's Degree in Economics from Middlebury College; Certified Health Coach",
  professionalBackground: [
    "Former Trader and Portfolio Manager at Point72 Asset Management",
    "Left finance to pursue passion in health and wellness",
  ],
  image:
    "https://maxpodcasting.com/wp-content/uploads/2024/06/Bill-Shufelt-Wild-Business-Growth-Podcast-293-Athletic-Brewing-Non-Alcoholic-Beer-Guy-768x512.jpg",
  socials: {
    twitter: "https://twitter.com/AthleticBrewing",
    linkedin: "https://www.linkedin.com/in/bill-shufelt-50b14b2/",
  },
  entrepreneurialCareer: {
    title: "Founder and CEO of Athletic Brewing Company",
    ventures: {
      preAthleticBrewing:
        "Identified a gap in the market for high-quality non-alcoholic beer while pursuing a healthier lifestyle",
      athleticBrewingLaunch:
        "Founded Athletic Brewing Company in 2017 with John Walker, launching first products in 2018",
    },
  },
  inspiration: [
    "Passion for fitness and a healthy lifestyle",
    "Desire for a flavorful non-alcoholic beer that complements an active life",
  ],
  publicPresence: [
    "Featured in Forbes 30 Under 30",
    "Speaker at industry conferences on entrepreneurship and non-alcoholic trends",
    "Active on social media promoting sober living and wellness",
  ],
};

export const distributionStages = [
  {
    year: "2017",
    stage: "Seed Funding and Product Development",
    requirements: [
      "Initial seed funding of $500k secured",
      "Development of core product lineup",
      "Establishment of brewing facility in Stratford, Connecticut",
    ],
    keyMetrics: [
      "Revenue: $53,000",
      "Product trials in seven stores across New England",
    ],
    enablers: [
      "Strong founder vision and industry expertise",
      "Focus on high-quality, craft non-alcoholic beer",
    ],
  },
  {
    year: "2018",
    stage: "Initial Retail Distribution",
    requirements: [
      "Product-market fit validation",
      "Establishment of relationships with local retailers",
      "Compliance with brewing regulations",
    ],
    keyMetrics: [
      "Sales through grocery and convenience stores",
      "40-60% of sales from e-commerce channels",
    ],
    enablers: [
      "Growing demand in the 'sober curious' market",
      "Effective online presence and direct-to-consumer sales",
    ],
  },
  {
    year: "2019",
    stage: "Series A Funding and Market Expansion",
    requirements: [
      "Series A funding of $3.1 million secured",
      "Scaling production capabilities",
      "Expansion into more U.S. states",
    ],
    keyMetrics: ["Revenue: $2.5 million", "Ambassador program launched"],
    enablers: [
      "Partnerships with Tastemaker Capital Partners and Blake Mycoskie",
      "Strong brand identity centered on wellness",
    ],
  },
  {
    year: "2020",
    stage: "Scaling Production and National Distribution",
    requirements: [
      "Series B funding of $17.5 million secured",
      "Acquisition of San Diego brewing facility",
      "Increased production capacity by 400%",
    ],
    keyMetrics: ["Revenue: $15 million", "Production of 37,500 barrels"],
    enablers: [
      "Award-winning products gaining recognition",
      "Expanded marketing efforts including TV advertising",
    ],
  },
  {
    year: "2021",
    stage: "International Expansion and Recognition",
    requirements: [
      "Series C funding of $50 million secured",
      "Expansion into international markets (Canada, UK, Australia)",
      "Enhanced brewing capacity and product offerings",
    ],
    keyMetrics: [
      "Revenue: $37 million",
      "Products available in 40 U.S. states",
    ],
    enablers: [
      "Named in TIME's '100 Most Influential Companies'",
      "Investment from celebrities like J.J. Watt",
    ],
  },
  {
    year: "2022",
    stage: "Scaling and Market Leadership",
    requirements: [
      "Series D funding of $75.5 million secured",
      "Opening of new 150,000 sq ft facility in Milford, CT",
      "Expansion to over 50,000 retailers",
    ],
    keyMetrics: [
      "Revenue: Approx. $74 million",
      "Annual production capacity of 6 million cases",
    ],
    enablers: [
      "Partnerships with Keurig Dr Pepper",
      "Growing ambassador network of 1,500 members",
    ],
  },
  {
    year: "2023",
    stage: "Product Diversification and Global Partnerships",
    requirements: [
      "Introduction of new products (Athletic Lite, DayPack Sparkling Water)",
      "Partnership with Arsenal Football Club",
      "Focus on direct-to-consumer relationships",
    ],
    keyMetrics: [
      "Continued revenue growth (exact figures undisclosed)",
      "Recognition as one of Fast Company's 'Most Creative Companies'",
    ],
    enablers: [
      "Strong DTC channel driving innovation",
      "Global brand recognition in non-alcoholic beer movement",
    ],
  },
  {
    year: "2024",
    stage: "Continued Expansion and Investment",
    requirements: [
      "Venture funding of $200 million secured from General Atlantic",
      "Doubling of production capacity with third brewing facility",
      "Tripling retail footprint in Canada",
    ],
    keyMetrics: [
      "Raised to date: $271.91 million",
      "Products available in major retailers like Walmart and Target",
    ],
    enablers: [
      "Strategic investments for sustainable growth",
      "Expanded international distribution channels",
    ],
  },
];

export const marketingStages = [
  {
    category: "Ambassador Program and Athlete Partnerships",
    requirements: [
      "Established brand identity focused on wellness",
      "Network within athletic and outdoor communities",
      "Marketing resources for ambassador engagement",
    ],
    examples: [
      {
        partnership: "Professional Athletes and Celebrities",
        conditions: [
          "Investments from J.J. Watt, Lance Armstrong, and others",
          "Brand ambassadors embodying active lifestyles",
        ],
      },
      {
        partnership: "NCAA Collegiate Athletes",
        conditions: [
          "Launch of ambassador program in 2019",
          "Engagement with younger, health-conscious demographics",
        ],
      },
    ],
  },
  {
    category: "Award Recognition and Media Features",
    requirements: [
      "High-quality products",
      "Participation in industry competitions",
      "PR and media outreach capabilities",
    ],
    examples: [
      {
        partnership: "International Beer Challenge",
        conditions: [
          "Winning awards like 'Supreme Champion Beer' in 2020",
          "Leveraging accolades for marketing purposes",
        ],
      },
      {
        partnership: "TIME Magazine and Inc. Magazine",
        conditions: [
          "Recognition as an influential and fast-growing company",
          "Utilizing media features to enhance brand credibility",
        ],
      },
    ],
  },
  {
    category: "Digital Marketing and DTC Engagement",
    requirements: [
      "Robust e-commerce platform",
      "Data analytics for customer insights",
      "Content creation resources",
    ],
    examples: [
      {
        partnership: "Fit For All Times Campaign",
        conditions: [
          "Featuring celebrities and athletes",
          "Connecting with Gen Z and millennials",
        ],
      },
      {
        partnership: "Social Media and Online-Only Products",
        conditions: [
          "Offering exclusive beers through online platform",
          "Engaging customers with limited-time offerings",
        ],
      },
    ],
  },
];

export const topInvestors = [
  {
    name: "General Atlantic",
    logo: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRru21QWK4tchjo-jiEzEi8tXJNSfaoj2TP3A&s",
    totalRoundsParticipated: 1,
    roundsParticipated: ["Deal #11"],
    leadInvestments: ["Deal #11"],
    otherInvestments: ["Airbnb", "Uber", "Slack"],
    website: "https://www.generalatlantic.com",
    description:
      "General Atlantic is a leading global growth equity firm providing capital and strategic support for growth companies.",
  },
  {
    name: "Keurig Dr Pepper",
    logo: "https://logosandtypes.com/wp-content/uploads/2023/03/Keurig-Dr-Pepper.png",
    totalRoundsParticipated: 1,
    roundsParticipated: ["Deal #8"],
    leadInvestments: [],
    otherInvestments: ["Core Hydration", "Bai Brands", "Polar Beverages"],
    website: "https://www.keurigdrpepper.com",
    description:
      "Keurig Dr Pepper is a leading beverage company in North America offering hot and cold beverages together at scale.",
  },
  {
    name: "Alliance Consumer Growth",
    logo: "https://pbs.twimg.com/profile_images/1059234316369022978/QpBiUNUn_400x400.jpg",
    totalRoundsParticipated: 2,
    roundsParticipated: ["Deal #6", "Deal #8"],
    leadInvestments: ["Deal #6"],
    otherInvestments: ["Krave Jerky", "EVOL Foods", "SkinnyPop Popcorn"],
    website: "https://www.allianceconsumergrowth.com",
    description:
      "Alliance Consumer Growth is a consumer-focused growth equity firm that invests in the most promising emerging brands.",
  },
  {
    name: "TRB Advisors",
    logo: "https://uspto.report/TM/85654457/mark.png",
    totalRoundsParticipated: 3,
    roundsParticipated: ["Deal #3", "Deal #4", "Deal #6"],
    leadInvestments: ["Deal #6"],
    otherInvestments: ["Beyond Meat", "Impossible Foods"],
    website: null,
    description:
      "TRB Advisors is a private investment firm founded by Timothy Barakett, focusing on venture capital and growth investments.",
  },
  {
    name: "Blake Mycoskie",
    logo: "https://pbs.twimg.com/profile_images/1234585013175250945/r19cdFYx_400x400.jpg",
    totalRoundsParticipated: 4,
    roundsParticipated: ["Deal #3", "Deal #4", "Deal #6", "Deal #7"],
    leadInvestments: [],
    otherInvestments: ["TOMS Shoes (Founder)", "Madefor"],
    website: "https://www.blakemycoskie.com",
    description:
      "Blake Mycoskie is an entrepreneur, philanthropist, and founder of TOMS Shoes, known for his 'One for One' business model.",
  },
];

export const ownershipData = [
  { date: "2017-09-13", foundersOwnership: 100, investorsOwnership: 0 },
  { date: "2019-12-09", foundersOwnership: 80, investorsOwnership: 20 },
  { date: "2020-03-20", foundersOwnership: 70, investorsOwnership: 30 },
  { date: "2021-05-11", foundersOwnership: 60, investorsOwnership: 40 },
  { date: "2022-11-15", foundersOwnership: 50, investorsOwnership: 50 },
  { date: "2024-08-09", foundersOwnership: 40, investorsOwnership: 60 },
];

export const investorParticipation = [
  {
    investor: "General Atlantic",
    "Deal #11": 1,
    "Deal #10": 0,
    "Deal #9": 0,
    "Deal #8": 0,
    "Deal #6": 0,
    "Deal #4": 0,
    "Deal #3": 0,
    "Deal #1": 0,
  },
  {
    investor: "Keurig Dr Pepper",
    "Deal #11": 0,
    "Deal #10": 0,
    "Deal #9": 0,
    "Deal #8": 1,
    "Deal #6": 0,
    "Deal #4": 0,
    "Deal #3": 0,
    "Deal #1": 0,
  },
  {
    investor: "Alliance Consumer Growth",
    "Deal #11": 0,
    "Deal #10": 0,
    "Deal #9": 0,
    "Deal #8": 1,
    "Deal #6": 1,
    "Deal #4": 0,
    "Deal #3": 0,
    "Deal #1": 0,
  },
  {
    investor: "TRB Advisors",
    "Deal #11": 0,
    "Deal #10": 0,
    "Deal #9": 0,
    "Deal #8": 1,
    "Deal #6": 1,
    "Deal #4": 1,
    "Deal #3": 1,
    "Deal #1": 0,
  },
  {
    investor: "Blake Mycoskie",
    "Deal #11": 0,
    "Deal #10": 0,
    "Deal #9": 0,
    "Deal #8": 1,
    "Deal #6": 1,
    "Deal #4": 1,
    "Deal #3": 1,
    "Deal #1": 0,
  },
];

export const valuationData = [
  { date: "2017-09-13", preMoneyValuation: 2.5, postMoneyValuation: 3 },
  { date: "2019-12-09", preMoneyValuation: 10, postMoneyValuation: 13 },
  { date: "2020-03-20", preMoneyValuation: 40, postMoneyValuation: 57.5 },
  { date: "2021-05-11", preMoneyValuation: 150, postMoneyValuation: 200 },
  { date: "2022-11-15", preMoneyValuation: 424.55, postMoneyValuation: 500 },
  { date: "2024-08-09", preMoneyValuation: 750, postMoneyValuation: 950 },
];

export const revenueData = [
  { year: 2017, revenue: 0 },
  { year: 2018, revenue: 0.053 },
  { year: 2019, revenue: 2.5 },
  { year: 2020, revenue: 15 },
  { year: 2021, revenue: 37 },
  { year: 2022, revenue: 74 },
  { year: 2023, revenue: null }, // Undisclosed
];

export const capitalRaisedData = [
  { date: "2017-09-13", cumulativeRaised: 0.5 },
  { date: "2018-10-02", cumulativeRaised: 0.88 },
  { date: "2019-12-09", cumulativeRaised: 3 },
  { date: "2020-03-20", cumulativeRaised: 17.5 },
  { date: "2021-05-11", cumulativeRaised: 80 },
  { date: "2022-11-15", cumulativeRaised: 155.45 },
  { date: "2024-08-09", cumulativeRaised: 355.45 },
];

export const revenueWithMilestones = [
  { year: 2017, revenue: 0, milestone: "Company Founded" },
  { year: 2018, revenue: 0.053, milestone: "First Products Launched" },
  { year: 2019, revenue: 2.5, milestone: "Series A Funding and Expansion" },
  { year: 2020, revenue: 15, milestone: "Acquired San Diego Facility" },
  { year: 2021, revenue: 37, milestone: "International Expansion" },
  { year: 2022, revenue: 74, milestone: "Opened Milford Facility" },
  { year: 2023, revenue: null, milestone: "Global Partnerships Established" },
];

export const businessEvents = [
  {
    year: 2018,
    category: "Product",
    event: "Launched Run Wild IPA and Upside Dawn Golden Ale",
  },
  {
    year: 2019,
    category: "Marketing",
    event: "Ambassador Program Initiated",
  },
  {
    year: 2020,
    category: "Expansion",
    event: "Acquired San Diego Brewing Facility",
  },
  {
    year: 2021,
    category: "Recognition",
    event: "Named in TIME's '100 Most Influential Companies'",
  },
  {
    year: 2022,
    category: "Funding",
    event: "Secured Series D Funding of $75.5 Million",
  },
  {
    year: 2023,
    category: "Product",
    event: "Introduced Athletic Lite and DayPack Sparkling Water",
  },
  {
    year: 2024,
    category: "Investment",
    event: "Raised $200 Million from General Atlantic",
  },
];

export const timelineData = [
  {
    date: "2017",
    eventName: "Seed Funding",
    category: "Funding",
    eventDescription:
      "Athletic Brewing Co. raised $500k in seed funding from angel investors to launch the brand.",
    additionalInfo: {
      dealInfo: {
        dealTypes: "Seed Round",
        dealDate: "13-Sep-2017",
        dealStatus: "Completed",
        financingSource: "Venture Capital",
      },
      companyInfo: {
        financingStatus: "Angel-Backed",
        businessStatus: "Startup",
        ceoLeadMGT: "William Shufelt",
        site: "Milford, CT",
      },
      totalCapital: {
        dealAmount: "£2.31M",
        raisedToDate: "£2.31M",
      },
      equity: {
        vcRound: "1st Round",
        totalInvestedEquity: "£2.31M",
      },
    },
  },
  {
    date: "2021",
    eventName: "Partnership with J.J. Watt",
    category: "Branding & Marketing",
    eventDescription:
      "NFL star J.J. Watt joined Athletic Brewing as an investor and brand ambassador, increasing the brand's visibility among sports enthusiasts.",
    additionalInfo: {
      campaignDetails: {
        ambassador: "J.J. Watt",
        impact:
          "Expanded Athletic Brewing’s reach within the fitness and sports community, appealing to active lifestyle consumers.",
        channels: ["Social Media", "Sports Events"],
      },
    },
  },
  {
    date: "2022",
    eventName: "Fit For All Times Campaign",
    category: "Branding & Marketing",
    eventDescription:
      "Launched the ‘Fit For All Times’ campaign featuring athletes and influencers to promote a lifestyle-friendly non-alcoholic beverage choice.",
    additionalInfo: {
      campaignDetails: {
        theme: "Fitness and Wellness",
        keyInfluencers: ["College Athletes", "Professional Athletes"],
        projectedImpressions: "200 million",
        outcome:
          "Strengthened brand’s health-conscious and lifestyle-oriented image.",
      },
    },
  },
  {
    date: "2022",
    eventName: "First Price Increase Due to Inflation",
    category: "Pricing",
    eventDescription:
      "Athletic Brewing raised prices by 5% to address inflationary pressures across their supply chain, marking the first increase since launch.",
    additionalInfo: {
      pricingDetails: {
        reason: "Inflation and increased production costs",
        priceImpact: "5% increase across most products",
        customerResponse:
          "Minimal backlash due to brand loyalty and perceived quality.",
      },
    },
  },
  {
    date: "2018",
    eventName: "Launch of Core Products",
    category: "Product",
    eventDescription:
      "Athletic Brewing launched its first core non-alcoholic craft beers: Run Wild IPA and Upside Dawn Golden Ale.",
    additionalInfo: {
      productDetails: {
        coreLineup: ["Run Wild IPA", "Upside Dawn Golden Ale"],
        keyFeatures: "Non-alcoholic, low-calorie, high-quality ingredients",
        targetMarket: "Health-conscious and active consumers",
      },
    },
  },
  {
    date: "2021",
    eventName: "Launch of Seasonal Beers",
    category: "Product",
    eventDescription:
      "Athletic Brewing began releasing limited-edition seasonal flavors, including Oktoberfest, expanding its product line.",
    additionalInfo: {
      productDetails: {
        seasonalOfferings: ["Oktoberfest"],
        productStrategy:
          "Create seasonal excitement and drive repeat purchases",
        salesImpact:
          "Increased sales during fall season, building loyal customers.",
      },
    },
  },
  {
    date: "2023",
    eventName: "Launch of DayPack Sparkling Water",
    category: "Product",
    eventDescription:
      "Expanded beyond beer with DayPack Sparkling Water, catering to health-conscious consumers seeking a broader range of beverages.",
    additionalInfo: {
      productDetails: {
        lineExtension: "DayPack Sparkling Water",
        targetMarket: "Health-conscious and non-alcoholic consumers",
        flavors: ["Lemon Lime", "Blood Orange"],
        uniqueSellingPoints: "Low-calorie, all-natural flavors.",
      },
    },
  },
  {
    date: "2019",
    eventName: "First Year Sales",
    category: "Revenue",
    eventDescription:
      "Athletic Brewing achieved $53,000 in sales in its first year, laying the foundation for future growth.",
    additionalInfo: {
      revenueDetails: {
        salesChannels: [
          "Direct-to-Consumer (DTC)",
          "Early retail partnerships",
        ],
        revenueGrowthDrivers: [
          "Strong consumer interest",
          "Niche market appeal",
        ],
      },
    },
  },
  {
    date: "2023",
    eventName: "Annual Revenue Exceeds $200 Million",
    category: "Revenue",
    eventDescription:
      "Athletic Brewing's annual revenue exceeded $200 million, reflecting its rapid growth and popularity in the non-alcoholic market.",
    additionalInfo: {
      revenueDetails: {
        keyFactors: [
          "Expanded retail distribution",
          "Brand partnerships",
          "Product line extension",
        ],
        revenueBreakdown: {
          DTC: "20%",
          Retail: "80%",
        },
      },
    },
  },
  {
    date: "2020",
    eventName: "Expansion to California and Texas",
    category: "Retail & Distribution",
    eventDescription:
      "Athletic Brewing expanded its distribution into major U.S. markets, including California and Texas.",
    additionalInfo: {
      distributionDetails: {
        newMarkets: ["California", "Texas"],
        strategy:
          "Expand in high-consumption states to capture larger market share",
        retailPartners: ["Local grocery chains", "Specialty stores"],
      },
    },
  },
  {
    date: "2022",
    eventName: "Availability in Over 50,000 U.S. Retailers",
    category: "Retail & Distribution",
    eventDescription:
      "Athletic Brewing expanded to over 50,000 retail locations nationwide, securing partnerships with major grocery chains.",
    additionalInfo: {
      distributionDetails: {
        majorRetailers: ["Walmart", "Target", "Whole Foods Market"],
        totalOutlets: "50,000+",
        distributionGoal: "Maximize accessibility and brand presence",
      },
    },
  },
  {
    date: "2023",
    eventName: "International Expansion to Canada and UK",
    category: "Retail & Distribution",
    eventDescription:
      "Athletic Brewing launched its products in Canada and the UK, extending its reach to international markets.",
    additionalInfo: {
      distributionDetails: {
        countries: ["Canada", "United Kingdom"],
        localRetailers: ["Tesco (UK)", "Loblaws, Sobeys (Canada)"],
        expansionStrategy:
          "Capitalize on growing global demand for non-alcoholic beverages",
      },
    },
  },
  {
    date: "2018",
    eventName: "Angel Funding",
    category: "Funding",
    eventDescription:
      "Athletic Brewing Co. raised £0.38M from angel investors to support initial operations and development.",
    additionalInfo: {
      dealInfo: {
        dealTypes: "Angel Round",
        dealDate: "02-Oct-2018",
        dealStatus: "Completed",
        financingSource: "Individual",
      },
      companyInfo: {
        financingStatus: "Angel-Backed",
        businessStatus: "Startup",
        ceoLeadMGT: "William Shufelt",
        site: "Milford, CT",
      },
      totalCapital: {
        dealAmount: "£0.38M",
        raisedToDate: "£2.70M",
      },
      equity: {
        vcRound: "Angel",
        totalInvestedEquity: "£0.38M",
      },
    },
  },
  {
    date: "2019",
    eventName: "Series A Funding",
    category: "Funding",
    eventDescription:
      "Athletic Brewing Co. raised £2.41M in Series A funding from Tastemaker Capital Partners, Blake Mycoskie, and TRB Advisors.",
    additionalInfo: {
      dealInfo: {
        dealTypes: "Series A",
        dealDate: "09-Dec-2019",
        dealStatus: "Completed",
        financingSource: "Venture Capital",
      },
      companyInfo: {
        financingStatus: "Venture Capital-Backed",
        businessStatus: "Startup",
        ceoLeadMGT: "William Shufelt",
        site: "Milford, CT",
      },
      totalCapital: {
        dealAmount: "£2.41M",
        raisedToDate: "£5.11M",
      },
      equity: {
        vcRound: "2nd Round",
        totalInvestedEquity: "£2.41M",
      },
    },
  },
  {
    date: "2020",
    eventName: "Series B Funding",
    category: "Funding",
    eventDescription:
      "Athletic Brewing Co. raised £13.79M in Series B funding, enabling the company to expand across the U.S. and invest in production capacity.",
    additionalInfo: {
      dealInfo: {
        dealTypes: "Series B",
        dealDate: "20-Mar-2020",
        dealStatus: "Completed",
        financingSource: "Venture Capital",
      },
      companyInfo: {
        financingStatus: "Venture Capital-Backed",
        businessStatus: "Generating Revenue",
        ceoLeadMGT: "William Shufelt",
        site: "Milford, CT",
      },
      totalCapital: {
        dealAmount: "£13.79M",
        raisedToDate: "£18.90M",
      },
      equity: {
        vcRound: "3rd Round",
        totalInvestedEquity: "£13.79M",
      },
    },
  },
  {
    date: "2021",
    eventName: "Series C Funding",
    category: "Funding",
    eventDescription:
      "Athletic Brewing Co. raised £57.63M in Series C funding led by Alliance Consumer Growth, supporting expansion and innovation efforts.",
    additionalInfo: {
      dealInfo: {
        dealTypes: "Series C",
        dealDate: "11-May-2021",
        dealStatus: "Completed",
        financingSource: "Venture Capital",
      },
      companyInfo: {
        financingStatus: "Venture Capital-Backed",
        businessStatus: "Generating Revenue",
        ceoLeadMGT: "William Shufelt",
        site: "Milford, CT",
      },
      totalCapital: {
        dealAmount: "£57.63M",
        raisedToDate: "£76.53M",
      },
      equity: {
        vcRound: "4th Round",
        totalInvestedEquity: "£57.63M",
      },
    },
  },
  {
    date: "2022",
    eventName: "Series D Funding",
    category: "Funding",
    eventDescription:
      "Athletic Brewing Co. secured £65.82M in Series D funding, with participation from Keurig Dr Pepper and Alliance Consumer Growth, boosting its U.S. and international presence.",
    additionalInfo: {
      dealInfo: {
        dealTypes: "Series D",
        dealDate: "15-Nov-2022",
        dealStatus: "Completed",
        financingSource: "Venture Capital",
      },
      companyInfo: {
        financingStatus: "Venture Capital-Backed",
        businessStatus: "Generating Revenue",
        ceoLeadMGT: "William Shufelt",
        site: "Milford, CT",
      },
      totalCapital: {
        dealAmount: "£65.82M",
        raisedToDate: "£168.96M",
      },
      equity: {
        vcRound: "6th Round",
        totalInvestedEquity: "£65.82M",
      },
    },
  },
  {
    date: "2024",
    eventName: "Later Stage VC",
    category: "Funding",
    eventDescription:
      "Athletic Brewing Co. raised £52.87M in later-stage venture funding led by General Atlantic, aiming to support global expansion and increased production.",
    additionalInfo: {
      dealInfo: {
        dealTypes: "Later Stage VC",
        dealDate: "09-Aug-2024",
        dealStatus: "Completed",
        financingSource: "Venture Capital",
      },
      companyInfo: {
        financingStatus: "Venture Capital-Backed",
        businessStatus: "Generating Revenue",
        ceoLeadMGT: "William Shufelt",
        site: "Milford, CT",
      },
      totalCapital: {
        dealAmount: "£52.87M",
        raisedToDate: "£233.69M",
      },
      equity: {
        vcRound: "7th Round",
        totalInvestedEquity: "£52.87M",
      },
    },
  },
];
